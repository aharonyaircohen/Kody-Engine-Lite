...
```
┌─────────────────────────────────────────┐
│ Project Memory (hall + room filtered)   │
│  .kody/memory/architecture.md           │
│  .kody/memory/conventions.md            │
│  .kody/memory/conventions_{room}.md     │
│  .kody/memory/diary_{stage}.jsonl       │
├─────────────────────────────────────────┤
│ Step File (repo-customized prompt)      │
│  .kody/steps/<stage>.md                 │
│  ├── Original engine prompt             │
│  ├── ## Repo Patterns (real examples)   │
│  ├── ## Improvement Areas (gaps to fix) │
│  └── ## Acceptance Criteria (checklist) │
│  (YAML frontmatter: name, tools, mode)  │
│                                         │
│  Fallback: prompts/<stage>.md (engine)  │
│  if .kody/steps/ not found              │
├─────────────────────────────────────────┤
...
**Step file resolution:** `readPromptFile()` checks `.kody/steps/<stage>.md` in the project directory first. If not found, it falls back to the engine's built-in `prompts/<stage>.md` with a warning. Run `kody-engine-lite bootstrap --force` to generate customized step files.

**Tiered context loading** (L0/L1/L2) reduces prompt token usage while maintaining quality:
- **L0** (~30-100 tokens): AAAK-compressed shorthand. For JSON: key fields only.
- **L1** (~150-500 tokens): All headings + first sentence of each section + bullet items.
- **L2**: Full untruncated content.

Each stage has a context policy specifying which tier to use per source (e.g., build stage gets `plan: L2` but `memory: L1`), which memory halls to include (e.g., taskify gets `facts` only), and room filtering based on task scope. See [Memory System](MEMORY.md) for details.

## Memory & Step Files

See [Memory System](MEMORY.md) for full documentation of halls, rooms, tiers, compression, contradiction detection, and stage diaries.

```
.kody/
├── memory/
│   ├── architecture.md         # facts hall — tech stack, frameworks, structure
│   ├── conventions.md          # conventions hall — global learnings (deduped, pruned)
...
└── steps/
    ├── taskify.md         # Customized task classification prompt
    ├── plan.md            # Customized planning prompt with repo architecture
    ├── build.md           # Customized build prompt with repo patterns & gaps
    ├── autofix.md         # Customized autofix prompt for this repo's toolchain
    ├── review.md          # Customized review checklist for this repo
    └── review-fix.md      # Customized review-fix prompt
```

**Step files** (`.kody/steps/`) are generated by `bootstrap` using AI analysis of the codebase. They contain the engine's default prompt plus three repo-specific sections: Repo Patterns, Improvement Areas, and Acceptance Criteria. These are the primary prompts used during pipeline execution — the engine's built-in `prompts/*.md` are only used as fallback. See [Features](FEATURES.md#repo-aware-step-files-kodysteps).

**architecture.md** — Created by `bootstrap`. An LLM analyzes the project and writes: language, framework, database, package manager, project structure, key directories.

**conventions.md** — Updated by `auto-learn.ts` after each successful run with deduplication and pruning. Extracts from verify.md (testing framework, coverage, pre-existing failures), review.md (import conventions, path aliases), context.md (ORM detection), and task.json (active directories). Room-specific learnings are also written to `conventions_{room}.md`.

**diary_{stage}.jsonl** — Written by `agent.ts` after each stage completes. Records compressed patterns (files created, test results, review findings) scoped to the task's room. Read back and injected as context before the same stage runs on the next task.
...
5. Add the stage name to `STEP_STAGES` in `src/bin/cli.ts` so `bootstrap` generates a customized step file

6. Update complexity skip rules in `src/pipeline/complexity.ts` if needed
