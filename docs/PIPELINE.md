...
If `.kody/tools.yml` declares tools matching the current stage, their skill content is injected into the prompt under an `## Available Tools` section. Tools are detected, set up, and injected automatically — no stage-specific code needed. See [Tools](TOOLS.md).
