...
export async function releaseCommand(input: ReleaseInput): Promise<void> {
  ...
  if (input.dryRun) {
    ...
    logger.info(`  [dry-run] Would create PR: ${releaseBranch} → ${defaultBranch}`)
  ...
  const pr = createPR(releaseBranch, defaultBranch, `chore: release v${newVersion}`, prBody)
  ...
  if (pr) {
    ...
    logger.info(`Merge it to open a PR: dev → main (E2E will gate this PR)`)
    logger.info(`After merging to main: kody-engine release --finalize`)
